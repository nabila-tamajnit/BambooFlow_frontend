...
import { ArrowRight, Leaf, Timer, Target } from "lucide-react";
        ...
        <main className="overflow-hidden">
            <section className="relative py-20 lg:py-32 px-6 lg:px-44 flex flex-col items-center text-center bg-gradient-to-b from-main-100/50 to-white">
                ...
                <p className="text-xl text-main-600 max-w-2xl mb-10 font-medium">
                    Bienvenue dans votre forêt personnelle. Gérez vos tâches, restez concentré avec le Pomodoro et nourrissez votre productivité.
                </p>

                <div className="flex flex-col sm:flex-row gap-4">
                    {/* Bouton principal adaptatif selon l'état de connexion */}
                    <button
                        onClick={handleCTA}
                        className="btn text-lg px-10 py-4 flex items-center gap-2 group"
                    >
                        {isConnect ? 'Accéder à ma forêt' : 'Commencer l\'aventure'}
                        <ArrowRight size={20} className="group-hover:translate-x-1 transition-transform" />
                    </button>

                    {!isConnect && (
                        <NavLink to="/auth/login" className="px-10 py-4 rounded-2xl font-bold text-main-700 hover:bg-main-100 transition-colors">
                            Se connecter
                        </NavLink>
                    )}

                    {isConnect && (
                        <NavLink to="/pomodoro" className="px-10 py-4 rounded-2xl font-bold text-main-700 hover:bg-main-100 transition-colors">
                            Zone Focus 🎋
                        </NavLink>
                    )}
                </div>
            ...
            <section className="py-20 px-6 lg:px-44">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    ...
                    <div className="p-8 rounded-[2rem] bg-white border border-main-100 shadow-sm hover:shadow-xl transition-all">
                        ...
                        <p className="text-main-600">Plus vous avancez, plus votre forêt grandit.</p>
                    </div>
                </div>
            </section>
        </main>
